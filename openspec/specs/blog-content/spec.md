...
### Notes
- Date schema implemented as `z.coerce.date()` to handle string dates from sync
- `published: false` posts excluded via `getCollection(..., ({ data }) => data.published !== false)`
- Pagination uses Astro built-in `paginate()` with 9 posts per page
- Empty archive shows "Aún no hay posts publicados. ¡Vuelve pronto!"
