...
### Notes
- Font choice: Luckiest Guy (headings) + Inter (body) via Google Fonts `<link>`
- Grid uses `repeat(auto-fill, minmax(300px, 1fr))` pattern with explicit breakpoints
- Link hover color uses `--color-cta` (fixed post-verify)
- All OG tags render conditionally — og:image only renders when an image is available
