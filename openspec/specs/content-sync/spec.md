...
### Notes
- Script location: `scripts/sync-vault.mjs`
- Uses NFD normalization for slug generation (strip accents, ñ→n)
- Missing image references are logged as warnings, not errors
- Wikilinks to known pages (`about`, `metas`) become internal links; unknown wikilinks are stripped to plain text
- Asset comparison uses file size; newer file wins on conflict
